...
public class GenerateREPOTransferRule extends ProductTransferRule {
	...
	public Vector<TransferRule> generateRules(Trade trade) {
		...
		try {
			...
			coupon = (Coupon) ((Vector) remoteProduct
					.getCoupon(product.getId())).elementAt(0);
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		RepoCashFlow cashFlow = pricing.generateCashFlow();
		...
		for (int i = 0; i < flows.size(); i++) {
			...
			/*
			 * below statement has moved inside every if condition. For repo
			 * subflows we do not set payment date. rule.set_settleDate(new
			 * DateU(flow.getPaymentDate()));
			 */
			...
			if (flow.getType().equalsIgnoreCase(transerTYPEPRINCIPAL)) {
				...
				// rule.set_seqNumber(); // this is set to know which is first
				// principal and second principal
			...
			} else if (flow.getType().equalsIgnoreCase(transerTYPEINTEREST)) {
			...
			} else if (!flow.getType().equalsIgnoreCase(ProductConstants.COMPOUNDEDINTEREST) && !flow.getType().equalsIgnoreCase(transerTYPEINTEREST)
					&& (!flow.getType().equalsIgnoreCase(transerTYPEPRINCIPAL))) {
				rule.set_transferType(transerTYPEPRINCIPAL);
				rule.set_settleDate(new DateU(flow.getPaymentDate()));
				// rule.set_seqNumber(1); // this is set to know which is first
				// principal and second principal
				if (trade.getType().equalsIgnoreCase(tradeTypeSELL)) {
					rule.set_payReceive(RECEIVE);
					rule.set_payerLegalEntityId(le.getId());
	...
	private void addFeesRule(Vector<Fees> fees, Vector<TransferRule> bondRules,
			LegalEntity po, Trade trade) {
		// TODO Auto-generated method stub
		if (fees == null) {
			commonUTIL.display("GenerateMMTransferRule",
					" No Fees Attached for Trade " + trade.getId());
			return;
		}
		commonUTIL.display("GenerateMMTransferRule",
				" Started Processing transfers for Fees on " + trade.getId());
		for (int f = 0; f < fees.size(); f++) {
			...
			if (fee.getPayrec().trim().equalsIgnoreCase(PAY)) {
				...
				rulef.set_settleDate(new DateU(commonUTIL.stringToDate(
						fee.getFeeDate(), true)));
				...
				rulef.set_payerLegalEntityRole(getLegalEntity(fee.getLeID())
						.getRole());
			...
			commonUTIL.display(
					"GenerateMMTransferRule",
					" End of  Processing transfers for Fees on "
							+ trade.getId() + " attached ");
		}
