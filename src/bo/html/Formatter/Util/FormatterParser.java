...
public class FormatterParser 
	extends Object 
	{
	     ...
	     * 
	     */
	    private static HTMLFormatter  _formatter;
	    ...
	    static ThreadLocal<Stack<StringBuilder>> calypsoBlocksBeingScanned = new ThreadLocal<Stack<StringBuilder>>() {
	     ...
	     */
	    static ThreadLocal<Stack<StringBuilder>> calypsoBlocksCompletelyScanned = new ThreadLocal<Stack<StringBuilder>>() {
	     ...
	     *                phase has completed.
	     */
	    public static synchronized String parse(HTMLFormatter formatter,
	    ...
	    protected static void start(String s)
	    {
	    	...
	    	s = s.trim();
	    	int length = s.length();
	    	if (s.toUpperCase().endsWith("<!--CALYPSO>"))
	    	    s = s.substring(0, length-12);
	    	else if (s.toUpperCase().endsWith("<!--CALYPSO:R>"))
	            s = s.substring(0, length-14);
	        else if (s.toUpperCase().endsWith("<CALYPSO>"))
	     ...
	     */
	    public static void echoCalypso(String s) {
	    	System.out.println("FormatterP  echoCalypso  " + s);
	        calypsoBlocksBeingScanned.get().peek().append(s);
	    }
	    public static void startCalypsoLevel() {
	        StringBuilder sb = new StringBuilder();
	        calypsoBlocksBeingScanned.get().push(sb);
	    }
	    public static void endCalypsoLevel() {
	    	String dd = calypsoBlocksBeingScanned.get().pop().toString();
	    	 Stack<StringBuilder>  mm = new  Stack<StringBuilder>();
	    	 mm.add(new StringBuilder(dd));
	    	System.out.println(" endCalypsoLevel == "+ dd);
	    	 calypsoBlocksCompletelyScanned.get().add( mm.pop());
	    }
	    
	    public static void writeCalypsoBlock() {
	        Stack<StringBuilder> myStack = calypsoBlocksCompletelyScanned.get();
	        if (myStack.size() > 0) {
	            addOriginalLine("<!--generatedEnd--><!--calypso:r>"+myStack.peek().toString()+"</calypso-->");
	            myStack.pop();
	        } else {
	        	 commonUTIL.display(FORMATTER_PARSER, "Internal stack of scanned Calypso blocks is empty");
	        }
	    }
	    public static void dropCalypsoBlock() {
	        Stack<StringBuilder> myStack = calypsoBlocksCompletelyScanned.get();
	        if (myStack.size() > 0) {
	            myStack.pop();
	        } else {
	    ...
	    protected static void addLine(String s) {
	        ...
	        String finalString = _formatter.parse(s,
	                                              ...
	                                              _transferRules,
	                                              _transfer,
	                                              null,true,_productTransferRules);    	
	        _document.append(finalString);
		...
		InputStream stream = FormatterParser.class.getClassLoader().getResourceAsStream(fileName);

	String m = 	FormatterParser.parse(new HTMLFormatter(),
	                              stream,
	                              new Message(),
	                              new Transfer(),
	                              new Trade(),
