...
private void hierarachicalTableAction() {
	if (counterpartyWindow.hierarchicalTable.getSelectedRow() != -1) {
			counterpartyWindow.propertyTable.setPropertiesValues(counterpartyWindow.model.getRow(counterpartyWindow.hierarchicalTable.getSelectedRow()));
		counterpartyWindow.setLegalEntity(counterpartyWindow.model.getRow(counterpartyWindow.hierarchicalTable.getSelectedRow()));
		setLegalEntity(counterpartyWindow.model.getRow(counterpartyWindow.hierarchicalTable.getSelectedRow()));
		}
	}
private void saveButtonAction() {
counterpartyWindow.propertyTable
.setfillValues(counterparty);
setLegalEntity((LegalEntity) counterpartyWindow.propertyTable.getBean());
//if(validate( )) 
// if(ReferenceDataCache.updateCounterParty(getLegalEntity())) {
...
//if(validate( )){
// counterparty = ReferenceDataCache.saveLegalEntity(counterparty); 
...
public void loadData(int id) {
		newButtonAction();
		Vector<LegalEntity> data = null;// ReferenceDataCache.getLegalEntity(id);
			counterpartyWindow.model.clear();
			if (!commonUTIL.isEmpty(data)) {
				LegalEntity firstRecord = data.get(0);
				for (int i = 0; i < data.size(); i++) {
 ...
 private void rightSideCenterTableAction() {
	if(counterpartyWindow.rightSideCenterTable.getSelectedRow() != -1) 
		 counterpartyWindow.propertyTable.setPropertiesValues( counterpartyWindow.model.getRow(counterpartyWindow.rightSideCenterTable.getSelectedRow()));
	counterpartyWindow.setLegalEntity(counterpartyWindow.model.getRow(counterpartyWindow.rightSideCenterTable .getSelectedRow()));
...
private void deleteButtonAction() {
	try {
//if(ReferenceDataCache.deleteCounterParty(counterparty)) {
//if( counterpartyWindow.rightSideCenterTable.getSelectedRow() != -1) {
