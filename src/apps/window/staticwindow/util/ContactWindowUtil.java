...
public class ContactWindowUtil extends BaseWindowUtil {
	...
	private void hierarachicalTableAction() {
		if (contactWindow.hierarchicalTable.getSelectedRow() != -1) {
			contactWindow.propertyTable
					.setPropertiesValues(contactWindow.model.getRow(contactWindow.hierarchicalTable.getSelectedRow()));
	...
	private void saveButtonAction() {
		...
		// if(validate( ))
		 if(ReferenceDataCache.updateSQL(contact, BeanConstants.LECONTACTS)) {
		 if(contactWindow.rightSideCenterTable.getSelectedRow() != -1) {
		 int i= TableUtils.getSelectedRowIndex(
		 contactWindow.rightSideCenterTable);
	...
	private void saveAsNewButtonAction() {
		...
		// if(validate( )){
		 contact = (LeContacts) ReferenceDataCache.insertSQL(contact, BeanConstants.LECONTACTS);
	...
	private void loadButtonAction() {
		...
		if (commonUTIL.isEmpty(searchText)) {
			Vector<LeContacts> data =  (Vector<LeContacts>) ReferenceDataCache.selectWhere(searchText, BeanConstants.LECONTACTS);
			contactWindow.model.clear();
	...
	public void loadData(int id) {
		newButtonAction();
		Vector<LeContacts> data =  (Vector<LeContacts>) ReferenceDataCache.selectWhere(LeContactsConstants.SQL_ID + id, BeanConstants.LECONTACTS);
		contactWindow.model.clear();
		if (!commonUTIL.isEmpty(data)) {
			LeContacts firstRecord = data.get(0);
			for (int i = 0; i < data.size(); i++) {
	...
	private void rightSideCenterTableAction() {
		if (contactWindow.rightSideCenterTable.getSelectedRow() != -1)
			contactWindow.propertyTable.setPropertiesValues(
					contactWindow.model.getRow(contactWindow.rightSideCenterTable.getSelectedRow()));
	...
	private void deleteButtonAction() {
		try {
			 if(ReferenceDataCache.deleteSQL(contact, BeanConstants.LECONTACTS)) {
			 if( contactWindow.rightSideCenterTable.getSelectedRow() != -1) {
