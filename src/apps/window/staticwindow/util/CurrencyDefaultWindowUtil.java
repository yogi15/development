...
public class CurrencyDefaultWindowUtil extends BaseWindowUtil {
	...
	private void hierarachicalTableAction() {
		if (currencydefaultWindow.hierarchicalTable.getSelectedRow() != -1) {
			currencydefaultWindow.propertyTable.setPropertiesValues(
					currencydefaultWindow.model.getRow(currencydefaultWindow.hierarchicalTable.getSelectedRow()));
			currencydefaultWindow.setCurrencyDefault(
					currencydefaultWindow.model.getRow(currencydefaultWindow.hierarchicalTable.getSelectedRow()));
			setCurrencyDefault(
					currencydefaultWindow.model.getRow(currencydefaultWindow.hierarchicalTable.getSelectedRow()));
		}
	...
	private void saveButtonAction() {
		...
		// if(validate( ))
		 if(ReferenceDataCache.updateSQL(getCurrencyDefault(),CurrencyDefaultConstants.WINDOW_NAME)) {
		 if(currencydefaultWindow.rightSideCenterTable.getSelectedRow() != -1)
		 {
		 int i= TableUtils.getSelectedRowIndex(
		 currencydefaultWindow.rightSideCenterTable);
	...
	private void saveAsNewButtonAction() {
		...
		System.out.println(currencydefault.toString());
		currencydefaultWindow.propertyTable.setfillValues(currencydefault);
		setCurrencyDefault(currencydefault);
		// if(validate( )){
		 if(ReferenceDataCache.insertSQL(getCurrencyDefault(),CurrencyDefaultConstants.WINDOW_NAME) != null) {
			 if(currencydefaultWindow.rightSideCenterTable.getSelectedRow() != -1)
	...
	/*private void saveAsNewButtonAction() {
		...
		setCurrencyDefault(currencydefault);
		// if(validate( )){
		currencydefault = (CurrencyDefault) ReferenceDataCache.insertSQL(getCurrencyDefault(), currencydefaultName);
		currencydefaultWindow.model.addRow(getCurrencyDefault());
		 setCurrencyDefault(currencydefault);
		 }
		 */
	...
	private void rightSideCenterTableAction() {
		if (currencydefaultWindow.rightSideCenterTable.getSelectedRow() != -1)
			currencydefaultWindow.propertyTable.setPropertiesValues(
					currencydefaultWindow.model.getRow(currencydefaultWindow.rightSideCenterTable.getSelectedRow()));
		currencydefaultWindow.setCurrencyDefault(
				currencydefaultWindow.model.getRow(currencydefaultWindow.rightSideCenterTable.getSelectedRow()));
		setCurrencyDefault(
				currencydefaultWindow.model.getRow(currencydefaultWindow.rightSideCenterTable.getSelectedRow()));
	...
	private void deleteButtonAction() {
		try {
			 if(ReferenceDataCache.deleteSQL(getCurrencyDefault(),CurrencyDefaultConstants.WINDOW_NAME)) {
				 if(currencydefaultWindow.rightSideCenterTable.getSelectedRow() != -1) {
