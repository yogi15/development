...
import static apps.window.util.propertyUtil.CommonPropertyUtil.createCurrencyPairProperty;
...
public class PropertyGenerator {
	...
	public static List<Property> getProperties(String windowName,
		...
		try {
					...
					if (!commonUTIL.isEmpty(dataType) && !ws.isMapJavaObject()) {
						if ((dataType
							...
							if (!commonUTIL.isEmpty(sData)) {
								renumProperty = createStartUPDataProperty(
						...
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.INITIALDATA))) {
						...
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.MULTISELECTION))
								&& (ws.getIsStartupdata() == 1)) {
							Vector<String> sData = getStartUpData(ws
									.getStartUpDataName().trim());
							if (!commonUTIL.isEmpty(sData)) {
								renumProperty = createMultipleSelectionListProperty(
										ws.getWindowName(), ws.getFieldName(),
										ws.getCategory(), sData);
							}
						}
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.MULTICOLUMNNAMESSELECTION))) {
							renumProperty = getMultiColumnMethodNames(
									ws.getBeanName(), ws.getFieldName(),
									ws.getCategory());
						}
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.IFELSECONDITIONAL))) {
							Vector<String> sdata = new Vector<String>();

							renumProperty = createPropertyConditional(
									ws.getFieldName(), ws.getFieldName(),
									ws.getCategory(), sdata, windowName,
									ws.getDesignType());
						}
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.DATE))) {

							renumProperty = createDateProperty(
									ws.getFieldName(), ws.getFieldName(),
									ws.getCategory());
						...
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.HOLIDAYCODE))) {

							renumProperty = createHolidayCodeProperty(
									ws.getWindowName(), ws.getFieldName(),
									ws.getCategory());

						}
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.CURRENCYPAIR))) {
							Vector<CurrencyPair> sData = (Vector<CurrencyPair>) ReferenceDataCache.selectALLCurrencyPairWithContact( );
							if (!commonUTIL.isEmpty(sData)) {
								Vector<String> codeData = commonUTIL
										.convertCurrencyPairtoString(sData);
								renumProperty = createCurrencyPairProperty(
										ws.getWindowName(), ws.getFieldName(),
										ws.getCategory(), codeData);
							}

						}
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.BOOK))) {
							renumProperty = createBookProperty(
									ws.getFieldName(), ws.getFieldName(),
									ws.getCategory());
						}
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.TIMEZONE))) {

							renumProperty = createTimeZoneProperty(
									ws.getFieldName(), ws.getFieldName(),
									ws.getCategory());
						}
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.FOLDER))) {
							Vector<String> sData = getFolderData();
							if (!commonUTIL.isEmpty(sData)) {
								renumProperty = createFolderProperty(
										ws.getWindowName(), ws.getFieldName(),
										ws.getCategory(), sData);
							}
						}
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.LEGALENTITY))) {

							renumProperty = createLegalEntityProperty(
									ws.getFieldName(), ws.getFieldName(),
									ws.getCategory(), "PO");
						}
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.SEARCHBOX))) {

							renumProperty = createSearchBoxProperty(
									ws.getFieldName(), ws.getFieldName(),
									ws.getCategory(), "Product");
						}
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.ATTRIBUTE))) {
							Vector<String> attributeData = ReferenceDataCache
									.getStarupData(ws.getWindowName()
											+ "Attributes");
							AttributeContainer attributebean = new AttributeContainer();
							attributebean.setAttributeName(ws.getWindowName()
									+ "Attributes");
							attributebean
									.setAttributes(commonUTIL
											.convertStartupVectorToAtrributeVector(attributeData));
							renumProperty = createAttributeProperty(
						...
						if ((dataType
							...
							renumProperty = createStringProperty(ws
						...
						if ((dataType
								.equalsIgnoreCase(PropertyPaneConstants.FIELDNAMES))
								&& (ws.getIsStartupdata() == 0)) {
							renumProperty = createFieldListProperty(ws
									.getFieldName().trim(), ws.getFieldName()
									.trim(), ws.getCategory().trim(),
									ws.getWindowName());
						...
						if ((dataType.trim()
								.equalsIgnoreCase(PropertyPaneConstants.BOOLEAN))) {

							renumProperty = createBooleanProperty(ws
									.getFieldName().trim(), ws.getFieldName()
									.trim(), ws.getCategory().trim());
							if (ws.getFieldName().equalsIgnoreCase(
						...
						if ((dataType.trim()
								.equalsIgnoreCase(PropertyPaneConstants.INTEGER))) {
							renumProperty = createIntegerProperty(ws
									.getFieldName().trim(), ws.getFieldName()
									.trim(), ws.getCategory().trim());
						...
						if ((dataType.trim()
								.equalsIgnoreCase(PropertyPaneConstants.NUMBER))) {
						...
						if ((dataType.trim()
							...
							renumProperty = createPassWordProperty(ws
									.getFieldName().trim(), ws.getFieldName()
									.trim(), ws.getCategory().trim());
						}
						if ((dataType.trim()
								.equalsIgnoreCase(PropertyPaneConstants.BEANNAMES))) {
							Vector<String> sData = getBeanFiles(JavaFileGeneratorConstants.BEANSPATH);
							if (!commonUTIL.isEmpty(sData)) {
								sData.add("NONE"); // when we don't want to
													// select BEANNAME remove
													// method
								renumProperty = createBeanNameProperty(
										ws.getFieldName(), ws.getFieldName(),
										ws.getCategory(), sData);
								// renumProperty.setValue(ws.getDefaultValue()
								// );
						...
						if (renumProperty != null) {
							...
							if (commonUTIL.isEmpty(ws.getParentFieldName())) {
								if (ws.isCondition()
										&& !commonUTIL.isEmpty(ws
												.getConfigureIfelseCondition())) {

									Hashtable<String, String> conditions = getconditions(ws
											.getConfigureIfelseCondition());
									// renumProperty.addPropertyChangeListener(arg0,
									// arg1)
									addConditionalProperty(renumProperty,
											conditions);
								}
								propertyList.add(renumProperty);
							} else {
								Property parentProperty = getParentPropertyFromList(
										propertyList, ws.getParentFieldName());
								if (parentProperty != null) {
									addChildProperty(parentProperty,
											renumProperty);
								} else {
									commonUTIL.display(
											"PropertyGenerator",
											"Parent Property Field is Null on Window "
													+ ws.getWindowName()
													+ " for Child Property "
													+ ws.getFieldName()
													+ " dataType ==  "
			...
			commonUTIL.display(
					"PropertyGenerator",
					"No Properities found for this window on "
							+ ws.getFieldName() + " dataType ==  "
							+ ws.getDataType());
			return propertyList;
		}
	}

	private static void addConditionalProperty(Property conditionalProperty,
			final Hashtable<String, String> conditions) {
		conditionalProperty.addPropertyChangeListener(Property.PROPERTY_VALUE,
				new PropertyChangeListener() {
					...
					public void propertyChange(PropertyChangeEvent evt) {
						...
						List<Property> childList = (List<Property>) ((Property) evt
								.getSource()).getChildren();
								...
								if (!commonUTIL.isEmpty(childList)) {
									for (int i = 0; i < childList.size(); i++) {
										for (int s = 0; s < conds.length; s++) {
											if (conds[s]
													.equalsIgnoreCase(childList
															.get(i).getName())) {
												childList.get(i).setHidden(
														false);
	...
	public static Property createWindowFieldName(String windowName,
		...
		Property property = createFieldListProperty(fieldName, fieldName,
				category, windowName);
		return property;
	}
	...
	private static Property createHolidayCodeProperty(String windowName,
		...
		// TODO Auto-generated method stub
		Property renumProperty = null;
		Vector<HolidayCode> sData = (Vector<HolidayCode>) ReferenceDataCache
				.selectKeyColumns(HolidayCodeConstants.SELECTHOLIDAYCODECOLUMN,
						BeanConstants.HOLIDAY);

		Vector<String> codeData = commonUTIL.convertHolidayCodetoString(sData);
		if (!commonUTIL.isEmpty(sData)) {
			renumProperty = createMultipleSelectionListProperty(windowName,
					fieldName, category, codeData);
		}
		return renumProperty;
	}

	private static Property getParentPropertyFromList(
			List<Property> propertyList, String parentPropertyName) {
		...
		if (!commonUTIL.isEmpty(propertyList)) {
			...
			for (int i = 0; i < propertyList.size(); i++) {
				...
				if (parentProperty != null
						&& parentProperty.getName().equalsIgnoreCase(
								parentPropertyName)) {
					break;
				}
			}
	...
	public static Property getAttributeProperty(String AttributesName) {
		Vector<String> attributeData = ReferenceDataCache
				.getStarupData(AttributesName);
		AttributeContainer attributebean = new AttributeContainer();
		attributebean.setAttributeName(AttributesName);
		attributebean.setAttributes(commonUTIL
				.convertStartupVectorToAtrributeVector(attributeData));
		Property renumProperty = createAttributeProperty(
				attributebean.getAttributeName(),
				attributebean.getAttributeName(),
				attributebean.getAttributeName(), attributebean);
		// TODO Auto-generated method stub
		return renumProperty;
	}
	...
	private static void addChildProperty(Property parentProperty,
			Property childProperty) {
		if (parentProperty != null)
			parentProperty.addChild(childProperty);
		parentProperty.addPropertyChangeListener(Property.PROPERTY_VALUE,
				new PropertyChangeListener() {
					...
					public void propertyChange(PropertyChangeEvent evt) {
						...
						if (evt.getNewValue() instanceof Boolean) {
							Boolean parentSignal = (Boolean) evt.getNewValue();
							...
							if (parentSignal != null) {
								if (parentSignal.booleanValue() == true) {
									...
									if (!commonUTIL.isEmpty(childList)) {
										for (int i = 0; i < childList.size(); i++) {
											((Property) childList.get(i))
													.setHidden(false);
											// clearPropertyValue((Property)childList.get(i));
										}
									}
									((Property) evt.getSource())
											.setExpanded(true);
								}
								if (parentSignal.booleanValue() == false) {
									...
									if (!commonUTIL.isEmpty(childList)) {
										for (int i = 0; i < childList.size(); i++) {
											((Property) childList.get(i))
													.setHidden(true);
											clearPropertyValue((Property) childList
													.get(i));
										}
									}
									((Property) evt.getSource())
	...
	public static PropertyListMultipleSelection getMultiColumnMethodNames(
			String nameObject, String fieldName, String categoryName) {
		try {
			if (commonUTIL.isEmpty(nameObject)
					|| nameObject.equalsIgnoreCase("NONE"))
				return null;
				...
				for (int i = 0; i < windowProperty.size(); i++) {
					...
					if (!commonUTIL.isEmpty(dataType)) {
						...
						if ((dataType
							...
							if (!commonUTIL.isEmpty(sData)) {
								propertyList
										.add(createMultipleSelectionListProperty(
												ws.getWindowName(),
												ws.getFieldName(),
												ws.getCategory(), sData));
							}
