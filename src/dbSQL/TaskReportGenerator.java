...
//	System.out.println("Columns " + headers);
		 ...
		 Class<?> addType;
			try {
			  ...
			  addType = Class.forName(classType);
				...
				commonUTIL.displayError("TaskReportGenerator", "SetType", e);
			}
	...
	public Collection generateReportOnSQL(Vector resulstSetvector) throws RemoteException {
		...
		if(v1.size() > 0) {
			 ...
			 try {
				...
				// TODO Auto-generated catch block
				commonUTIL.displayError("TaskReportGenerator", "generateReportOnSQL" , e);
			}
	...
	// caching required for the above process
	...
	private void processtableData(ResultSet rs, int columnCount) {
		...
		try {
			...
			// TODO Auto-generated catch block
			commonUTIL.displayError("TaskReportGenerator", "processtableData" , e);
		}
